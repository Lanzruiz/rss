...
function displayVoiceChannel(channelID) {
  ...
  channelAudioRef.orderByKey().once('value',function(snapshot) {
    ...
    if(checkSnapshot == false) {
      ...
      snapshot.forEach(function(childSnapshot) {
          ...
          if (jQuery.inArray( audioDict["accessCode"], userTalk ) == -1) {
           ...
           //console.log("ptt_archived/{{Session::get('users_id')}}/"+channelID+"/"+audioDict["filename"]);
           //var storageRef = storage.ref("ptt_archived/{{Session::get('users_id')}}/"+channelID+"/"+audioDict["filename"]);
           if(ctr == 1) {
                //var storageRef = storage.ref("ptt_archived/{{Session::get('users_id')}}/"+channelID+"/"+audioDict["filename"]);
                // storageRef.getMetadata().then(function(metadata) {
                //   console.log(metadata);
                //       initialAudioByChannel.push({"channel": channelID, "filename": metadata, "access_code": audioDict['accessCode']});
                //  });
                //initialAudioByChannel.push({"channel": channelID, "filename": audioDict['data'], "access_code": audioDict['accessCode']});
                initialAudioByChannel.push({"channel": channelID, "filename": audioDict['voice_url'], "access_code": audioDict['accessCode']});
           } else {
            //  storageRef.getMetadata().then(function(metadata) {
            //         console.log(metadata)
            //        remainingAudioByChannel[0].audio.push({"channel": channelID, "filename": metadata, "access_code": audioDict['accessCode']});
            //   });
               //remainingAudioByChannel[0].audio.push({"channel": channelID, "filename": audioDict['data'], "access_code": audioDict['accessCode']});
               remainingAudioByChannel[0].audio.push({"channel": channelID, "filename": audioDict['voice_url'], "access_code": audioDict['accessCode']});
      ...
      for (i = 0; i <= initialAudioByChannel.length-1; i++) {
           ...
           $("#"+initialAudioByChannel[i].channel).attr('src',initialAudioByChannel[i].filename);
            ...
            if (audioCounter <=  remainingAudioByChannel[0].audio.length -1) {
              var currentAccessCode = remainingAudioByChannel[0].audio[audioCounter].access_code;
                ...
                //console.log('accesscode'+currentAccessCode);
                 ...
                 $("#"+initialAudioByChannel[0].channel).attr('src',remainingAudioByChannel[0].audio[audioCounter].filename);
                 $("#"+initialAudioByChannel[0].channel).trigger('play');
                 ...
                 $("#"+initialAudioByChannel[0].channel).attr('src',initialAudioByChannel[0].filename);
                 audioCounter = 0;
