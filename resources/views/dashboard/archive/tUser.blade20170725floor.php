			...
			console.log("Invalid Token Authentication.");
					...
					if(response == 'NoMoreEvent'){
						$('#tu_event').html("<p>No events found</p>");
						  @if($alertInOff <= 0)
									location.href="https://livewitnessapp.net/dashboard/archived";
							@endif
							...
							$.each(fixturesData,function(key,item){
								...
								if(ectr == 1) {
									...
									//$("#deleteSelectedEventTransport").attr('href','http://livewitnessapp.net/dashboard/delete-selected-event/'+item.user_code+'/'+item.events);
									...
									thedata += "<li type='uevent' tuuser_code='"+item.user_code+"' uevent_id='"+ item.alert_detail_id +"' tuevent_id='"+ item.events +"'>"+"Event - "+item.events+"</li>";
...
$(document).on("click","li[type='uevent']", function(e){
	...
	//$("#deleteSelectedEventTransport").attr('href','http://livewitnessapp.net/dashboard/delete-selected-event/'+tusercode+'/'+GLOBAL_TUSER_EVENT);
											...
											$.each(fixturesData,function(key,item){
												...
												if(ectr == 1) {
													if($('#tu_event li').hasClass('activeClass')) {
															thedata += "<li tuuser_code='"+item.user_code+"' type='uevent' uevent_id='"+ item.alert_detail_id +"' tuevent_id='"+ item.events +"'>"+"Event - "+item.events+"</li>";
													} else {
														  thedata += "<li class='activeClass' tuuser_code='"+item.user_code+"' type='uevent' uevent_id='"+ item.alert_detail_id +"' tuevent_id='"+ item.events +"'>"+"Event - "+item.events+"</li>";
													}
	...
	if(frameVideoClassCounter == 0) {
	  ...
	  var frameEventIntClassCounter = $('.tUserEventInt'+eventPopupID).length;
		...
		if(frameEventIntClassCounter == 0) {
				$("#transportEventIntelligence").append("<div id='tUserEventInt' class='tUserEventInt"+eventPopupID+"'><div class='col-lg-12 header' align='right'><span class='btn btn-warning btn-sm'>Event Intelligence - Transport: "+eventRec.user_name+" <span style='color: #006E2E !important'><strong>Event #"+eventRec.events+"</strong></span></span><span id='tuEvClose"+eventPopupID+"' onclick='tranportEventClose("+eventPopupID+")' class='btn btn-danger btn-sm'>X</span></div><div class='col-lg-12'><div id='tuEvContent"+eventPopupID+"'>&nbsp;&nbsp;Location:&nbsp;&nbsp;<span class='tuser-location"+eventPopupID+"'>"+eventRec.location_now+"</span><hr>&nbsp;&nbsp;Event:&nbsp;&nbsp;<span class='tuser-event"+eventPopupID+"'>"+eventRec.events+"</span><hr>&nbsp;&nbsp;TDS:&nbsp;&nbsp;<span class='tuser-tds"+eventPopupID+"'>"+eventRec.alert_datetime+"</span><hr>&nbsp;&nbsp;Speed:&nbsp;&nbsp;<span class='tuser-speed"+eventPopupID+"'>"+eventRec.alert_speed+" mph</span><hr>&nbsp;&nbsp;Elevation:&nbsp;&nbsp;<span class='tuser-elevation"+eventPopupID+"'>"+eventRec.elevation+" ft</span><hr>&nbsp;&nbsp;Direction:&nbsp;&nbsp;<span class='tuser-direction"+eventPopupID+"'>"+eventRec.direction+"</span><hr>&nbsp;&nbsp;Floor:&nbsp;&nbsp;<span class='tuser-floor"+eventPopupID+"'>"+eventRec.floor+"</span><hr>&nbsp;&nbsp;Battery Level:&nbsp;&nbsp;<span class='tuser-battery-level"+eventPopupID+"'>"+eventRec.alert_battery_level+"</span><hr>&nbsp;&nbsp;Latitude:&nbsp;&nbsp;<span class='tuser-lat"+eventPopupID+"'>"+eventRec.lat+"</span><hr>&nbsp;&nbsp;Longitude:&nbsp;&nbsp;<span class='tuser-lng"+eventPopupID+"'>"+eventRec.lng+"</span><hr></div></div></div>");
...
function streamingArchiveTransport(src){
	...
	swfobject.embedSWF('https://livewitnessapp.net/public/assets/player.swf','video','100%','200','10.2',null,flashvars,params,attributes,embedHandler);
...
function streamingArchiveTransportPopup(src,accesscode,eventPopupID) {
	...
	swfobject.embedSWF('https://livewitnessapp.net/public/assets/player.swf','mainVideo'+eventPopupID,'100%','100%','10.2',null,flashvars,params,attributes);
}
	 ...
	 console.log("Transport Load New Events");
		...
		transportVideoRef.on('value', function(snapshot){
						if(GLOBAL_REFRESH == true) {
							...
							// 	if(snapDict["refresh_status"] == "true") {
							// 		 transportVideoRef.child(key).update({ refresh_status : "false" });
							// 		 //location.href = "http://livewitnessapp.net/dashboard/archived";
							// 		  refresh = true;
							//
							// 	}
	...
	if(c==true) {
		 ...
		 var events = $("#deleteSelectedEventTransport").data("events");
		 				...
		 				$.ajax({
							type: "GET",
							headers: { "Authorization": "{{Session::get('securityToken')}}", "Content-Type": "application/x-www-form-urlencoded" },
							url:  "https://livewitnessapp.net/dashboard/delete-selected-event/"+accesscode+"/"+events,
							cache: false,
							success: function(){
								$("#tu_event").empty();
								$("#video_feed").empty();
    ...
    <div class="col-lg-2" id="transDataFrequency">
        ...
        <div class="tu-last-frequency">
            ...
            &nbsp;&nbsp;Direction:&nbsp;&nbsp;<span class="tuser-direction"></span><hr>
						&nbsp;&nbsp;Floor:&nbsp;&nbsp;<span class="tuser-floor"></span><hr>
