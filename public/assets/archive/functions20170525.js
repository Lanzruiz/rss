...
function jf_tusers(){
	...
	for(i = 0; i < jv_tusers.length; i++){
		tusers = "<option value='"+jv_tusers[i]['user_id']+"' tuser_sel_event='"+jv_tusers[i]['events']+"'>"+jv_tusers[i]['user_name']+"</option>";
...
function jf_tu_events(){
	...
	for(i = 0; i < jv_tu_event.length; i++){
		...
		tuevent = "<li type='uevent' uevent_id='"+ jv_tu_event[i]['events'] +"'>"+"Event - "+jv_tu_event[i]['events']+"</li>";
...
function jf_tu_videos(){
	...
	if(jv_tu_video==''){
		...
		for(i = 0; i < jv_tu_video.length; i++){
			tuvideo = "<li type='tuvideo' tuvideo_id='"+ jv_tu_video[i]['alert_detail_id'] +"'>"+jv_tu_video[i]['alert_datetime']+"</li>";
			document.write(tuvideo);
		}
	}
}