...
class UserController extends Controller
{
    ...
    public function checkAccess() {
	     ...
	     if ($validator->fails()) {
          ...
          $listUsers = Users::where('fldUserLevel','!=','commander')
                              ->where('fldUsersCommanderID','=',Session::get('users_id'))
                              ->get();
          ...
          if(count($listUsers) >0) {
            ...
            foreach($listUsers as $listUserss) {
              //$imageStorage = DataStorage::CountImageDataStorage(Session::get('user_id'),$listUserss->fldUsersAccessCode);
              //$audioStorage = DataStorage::CountAudioDataStorage(Session::get('user_id'),$listUserss->fldUsersAccessCode);
              //$total_video_storage = DataStorage::CountVideoDataStorage($listUserss->fldUserID);
              //$totalStorage = $imageStorage + $audioStorage;
              //$total_data_usage = $total_data_usage + $total_video_storage;
              //$total_data_usage = $total_data_usage + $totalStorage;
            ...
            if($convertGB > 1) {
               ...
               $subscription = Subscription::where('user_id','=',Session::get('user_id'))
          ...
          //************check  user data usage**************/
              ...
              // if($users->fldUserSCStatus == 1 || $users->fldUserSCStatus == NULL) {
              //       $security_code = rand(100000,999999);
              //       //update user security code
              //       Users::updateSecurityCode($users->fldUserID,$security_code);
              //
              //       //send email
              //       $to = $users->fldUsersEmail;
				      //       $txt = "Hi ".ucwords($users->fldUsersUserName).", Your ".PRODUCT_TITLE." security code is: $security_code";
				      //       $subject = PRODUCT_TITLE." Security Code";
              ...
              //
              //       //send SMS
              //       $message = "Hi ".ucwords($users->fldUsersUserName).", your LiveWitness security code is: ".$security_code;
    ...
    public function updateProfile() {
        ...
        $users = Users::find(Session::get('users_id'));
         ...
         $rules   = Users::rules(Session::get('users_id'));
         $validator = Validator::make(Input::all(), $rules);

         if ($validator->fails()) {
               return Redirect::to('dashboard/profile')->withInput()->withErrors($validator,'users');
         } else {
             Users::AddUpdateUser(Session::get('users_id'));
    ...
    public function displayArchive() {
         ...
         $agent = Users::where('fldUsersCommanderID','=',Session::get('users_id'))
                        ->where('fldUserLevel','=','agent')
                        ->get();

         $transport = Users::where('fldUsersCommanderID','=',Session::get('users_id'))
        ...
        if($alertDetails < 1 && $alertInOff < 1) {
         ...
         $usersAlert = AlertDetails::displayUsers(Session::get('users_id'),0);
         $displayAlertDetail = AlertDetails::displayClientStatus(Session::get('users_id'),1);
         $client_id = Session::get('users_id');
   ...
   public function CleanAllFeeds() {
        ...
        $allUser = Users::where('fldUsersCommanderID','=',Session::get('users_id'))
                                ->get();
